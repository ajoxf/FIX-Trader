...
# -- which way a contract may be traded -------------------------------------

def test_short_only_sells_a_rich_spread_and_passes_over_a_cheap_one():
    """A spread ABOVE its mean is SOLD. A desk that will only sell — because
    the other side is the one that gaps, or it can only borrow one way —
    sets this, and the long-spread entry is never taken."""
    short_only = cfg(trade_direction='SHORT_ONLY')
    rich = entry(window(), 2.5, short_only)
    assert rich.action == 'OPEN' and rich.side is Side.SELL

    cheap = entry(window(), -2.5, short_only)
    assert cheap.action == 'NONE'
    assert 'SHORT spread only' in cheap.blocked_by

    both = entry(window(), -2.5, cfg())                      # the control
    assert both.action == 'OPEN' and both.side is Side.BUY


def test_long_only_buys_a_cheap_spread_and_passes_over_a_rich_one():
    long_only = cfg(trade_direction='LONG_ONLY')
    cheap = entry(window(), -2.5, long_only)
    assert cheap.action == 'OPEN' and cheap.side is Side.BUY

    rich = entry(window(), 2.5, long_only)
    assert rich.action == 'NONE' and 'LONG spread only' in rich.blocked_by

    both = entry(window(), 2.5, cfg())                       # the control
    assert both.action == 'OPEN' and both.side is Side.SELL


def test_the_direction_rule_never_withholds_an_exit():
    """The rule that matters. A filter that could withhold a close would
    strand exactly the position the desk was most careful about."""
    short_only = cfg(trade_direction='SHORT_ONLY', exit_signal_mode='zscore')
    w = window()
    held = position(Side.SELL, target=w.price_at_z(0.4))
    assert exit_for(w, 0.0, held, short_only).action == 'CLOSE'

    # and a LONG held on a short-only contract — a recovered book, or a
    # setting changed under an open position — still leaves
    w2 = window()
    stray = position(Side.BUY, target=w2.price_at_z(-0.4))
    out = exit_for(w2, 0.0, stray, short_only)
    assert out.action == 'CLOSE' and out.side is Side.SELL


def test_the_stop_still_fires_on_a_one_way_contract():
    """The stop is the exit that must never be missed, and it is not an
    entry."""
    short_only = cfg(trade_direction='SHORT_ONLY', stop_loss_z=3.0)
    w = window()
    held = position(Side.SELL, target=w.price_at_z(0.4))
    sig = exit_for(w, 3.6, held, short_only)
    assert sig.action == 'CLOSE' and sig.exit_reason is ExitReason.STOP_LOSS


def test_an_unrecognised_direction_is_treated_as_both():
    """A setting nobody typed, or one from a newer build. Falling back to
    BOTH is the behaviour that existed before the rule did — never a silent
    refusal to trade at all."""
    for value in ('', None, 'anything-else'):
        sig = entry(window(), 2.5, cfg(trade_direction=value))
        assert sig.action == 'OPEN', value
